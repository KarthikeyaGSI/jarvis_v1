"""
JARVIS SUPER AGENT - WISPR FLOW EDITION
Features learned from Wispr Flow:
1. Smart Auto-Edit - Removes filler words, fixes typos, formats text
2. Personal Dictionary - Learns your unique words/names
3. Snippet Library - Voice shortcuts for repeated phrases  
4. App-Specific Tones - Auto-adjusts tone per app
5. Multi-Language - 100+ languages with auto-detection
6. Real-Time Polish - Shows clean text after you finish speaking
"""
import os
import sys
import time
...
print("\n" + "="*60)
print("  JARVIS - WISPR FLOW EDITION")
print("="*60)
print(f"[INIT] Working from: {SCRIPT_DIR}")
print("[INIT] Loading Wispr Flow features...\n")
...
def learn_from_text(text):
    """Auto-learn words from context"""
    words = text.split()
    for word in words:
        if len(word) > 6 and word not in personal_dict:
            # Simple heuristic: long words might be names/jargon
            if word.istitle() or any(c.isupper() for c in word):
                personal_dict[word.lower()] = {
                    'word': word,
                    'added_at': datetime.now().isoformat()
                }
...
def expand_snippet(text):
    """Replace snippet cues with full text"""
    words = text.split()
...
def detect_app_tone():
    """Detect current active app and return appropriate tone"""
    try:
        import pyautogui
        # This is a simplified version - in reality you'd use Windows API
...
def polish_speech(raw_text):
    """Wispr Flow-style: Remove fillers, fix typos, format text"""
    if not ollama_available:
        return raw_text
    
    # Expand snippets first
    text = expand_snippet(raw_text)
    
    # Check dictionary for custom words
    words = text.split()
    for i, word in enumerate(words):
        if word.lower() in personal_dict:
            words[i] = personal_dict[word.lower()]['word']
    text = ' '.join(words)
...
# ========= ENHANCED FLOW MODE (WISPR STYLE) =========
def flow_mode(duration=30):
    """Wispr Flow Mode: Natural speech → AI-polished text → Auto-type"""
    if not stt_model:
        return "STT not available for Flow Mode, Sir."
    
    print(f"\n[WISPR FLOW] Speak naturally for {duration} seconds...")
    print("[FLOW] Filler words will be removed, text polished, typed anywhere!")
    ...
    try:
        ...
        # Detect active app and adjust tone
        tone = detect_app_tone()
        if tone != 'neutral':
            polished = adjust_tone(polished, tone)
            print(f"[FLOW] Adjusted tone for {tone} app")
        ...
        if pyautogui:
            ...
            return f"Wispr Flow: Typed polished text, Sir."
        else:
            return f"Wispr Flow: Polished text ready - {polished}"
...
def process_command(text):
    ...
    if 'help' in text_lower:
        return """JARVIS - WISPR FLOW EDITION COMMANDS:
...
INPUT: Type [text], Click, Clipboard
WISPR FLOW:
  - flow mode (speak & type polished text)
  - quick flow (15 sec quick mode)
  - add word [X] to dictionary
  - add snippet [cue] = [full text]
  - show dictionary / show snippets
AI: Ask me anything! (Ollama powered)
SCHEDULE: Remind me, Run at, Repeat every
UTILS: Battery, CPU, RAM, History, Help"""
...
def main():
    print("\n" + "="*60)
    print("  WISPR FLOW EDITION STATUS")
    print("="*60)
    ...
    print("="*60)
    print("\n  NEW: Wispr Flow features - Smart Auto-Edit, Dictionary, Snippets!")
    print("="*60 + "\n")
    
    speak("JARVIS Wispr Flow edition online. Smart features ready, Sir.")
