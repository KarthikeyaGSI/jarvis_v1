"""
JARVIS SUPER AGENT - WITH FLOW MODE (Wispr Flow clone)
- AI Brain (Ollama phi3:mini)
- FLOW MODE: Voice-to-text with AI polishing
...
print("\n" + "="*60)
print("  JARVIS SUPER AGENT - WITH FLOW MODE")
print("="*60)
print(f"[INIT] Working from: {SCRIPT_DIR}")
print("[INIT] Loading capabilities...\n")
...
def polish_speech(raw_text):
    """AI polish raw speech - remove fillers, fix typos"""
    if not ollama_available:
        return raw_text
    try:
        prompt = f"Polish this speech. Remove filler words (um, uh, like, you know). Fix typos. Make it clear and professional: {raw_text}"
        response = ollama.chat(model=ollama_model, messages=[
...
def flow_mode(duration=30):
    """Flow Mode: Speak naturally, AI polishes, types anywhere"""
    if not stt_model:
        return "STT not available for Flow Mode, Sir."
    
    print(f"\n[FLOW MODE] Speak naturally for {duration} seconds - AI will polish your text!")
    print("[FLOW] Filler words will be removed, typos fixed...")
    ...
    try:
        ...
        print(f"[FLOW] Polished with AI: {polished[:100]}...")
...
INPUT: Type [text], Click, Mouse position
UTILS: Clipboard, Battery, CPU, RAM, History
...
SCHEDULE: 
  ...
  - Remove task [id]
MODE: Say 'voice' for voice mode, 'text' for text"""
...
def main():
    ...
    print("="*60)
    print("\n  NEW: FLOW MODE - AI-polished voice-to-text!")
    print("="*60 + "\n")
    
    speak("JARVIS Super Agent online. Flow Mode ready, Sir.")
